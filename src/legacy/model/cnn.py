...
def max_2x2pool(incoming, name):
  ...
  with tf.variable_scope(name):
    return tf.nn.max_pool(incoming, ksize=(1, 2, 2, 1), strides=(1, 2, 2, 1),
                          padding='VALID')
...
def max_2x1pool(incoming, name):
  ...
  '''
  with tf.variable_scope(name):
    return tf.nn.max_pool(incoming, ksize=(1, 2, 1, 1), strides=(1, 2, 1, 1),
                          padding='VALID')
...
def ConvRelu(incoming, num_filters, filter_size, name):
  ...
  with tf.variable_scope(name):
    ...
    after_conv = tf.nn.conv2d(incoming, conv_W, strides=(1, 1, 1, 1),
                              padding='SAME')
...
def batch_norm(incoming, is_training):
  ...
  :return:
  '''
  return tf.contrib.layers.batch_norm(incoming, is_training=is_training,
...
def dropout(incoming, is_training, keep_prob=0.5):
  return tf.contrib.layers.dropout(incoming, keep_prob=keep_prob,
...
class CNN(object):
  ...
  def _build_network(self, input_tensor, is_training):
    ...
    print('CNN outdim before squeeze: {}'.format(
      net.get_shape()))  # 1x32x100 -> 24x512
