...
class Seq2SeqModel(object):
  ...
  def __init__(self, encoder_masks, encoder_inputs_tensor,
    ...
    cell = _create_cell(attn_num_hidden, attn_num_layers, use_gru)

    # The seq2seq function: we use embedding for the input and attention.
    def seq2seq_f(lstm_inputs, decoder_inputs, seq_length, do_decode):
      num_bi_layers = int(attn_num_layers / 2)

      num_hidden = num_bi_layers * attn_num_hidden
      lstm_fw_cell = tf.contrib.rnn.BasicLSTMCell(num_hidden, forget_bias=0.0,
                                                  state_is_tuple=False)
      # Backward direction cell
      lstm_bw_cell = tf.contrib.rnn.BasicLSTMCell(num_hidden, forget_bias=0.0,
                                                  state_is_tuple=False)
      ...
      encoder_inputs = [e * f for e, f in
                        zip(pre_encoder_inputs, encoder_masks[:seq_length])]
      top_states = [array_ops.reshape(e, [-1, 1, num_hidden * 2])
                    for e in encoder_inputs]
      attention_states = array_ops.concat(top_states, 1)
      initial_state = tf.concat(axis=1,
                                values=[output_state_fw, output_state_bw])
